...
export default function RevenueChart({
  ...
  subtitle = "Visualização semestral",
  tooltipLabel,
  legendLabel = "Receita Realizada",
}: {
  data: ChartDataPoint[];
  title?: string;
  subtitle?: string;
  ...
  const formatTooltip = tooltipLabel ?? ((v: number) => `R$ ${v}k`);
    ...
    <div className="rounded-2xl border border-slate-800 bg-slate-900/50 p-6 shadow-sm">
      ...
      <div className="mt-6 flex items-center justify-between border-t border-slate-800 pt-4">
        <div className="flex items-center gap-2">
          ...
          <span className="text-xs text-slate-300">{legendLabel}</span>
        </div>
      </div>
    </div>
  );
